...
constexpr const char* kModVersion = "v1.0.02z1-gog";
...
constexpr DWORD kPrimaryActorTargetSetterOffset = 0x00124AF0;
constexpr DWORD kSecondaryActorTargetSetterOffset = 0x00124C10;
...
constexpr DWORD kCeguiSetVisibleIatStatic = 0x0211612C;
constexpr DWORD kCeguiSetTextIatStatic = 0x02116228;
...
void LogMessageProbeCall(void* core, void* message, void* callerReturn) {
    ...
    if (!ReadDwordSafe(message, 0x00, &vftable) || !ReadByteSafe(message, 0x04, &code)) {
        if (kVerboseHookDiagnostics) {
            LogLine("Message send GOG 0x6E18B0 skipped: message header is unreadable.");
        }
        return;
    }
    constexpr DWORD kOriginalImageBase = 0x00400000;
    constexpr DWORD kSetRunningStatic = 0x021475BC;
    constexpr DWORD kSpawnStatic = 0x021475EC;
    constexpr DWORD kPostSpawnSettingsStatic = 0x021473AC;
    constexpr DWORD kRequestLevelOwnershipStatic = 0x021439DC;
    constexpr DWORD kSendLevelOwnershipStatic = 0x021473BC;
    constexpr DWORD kRerollLevelStatic = 0x02143AAC;
    constexpr DWORD kSetLevelStatic = 0x021476CC;
...
bool LooksLikeReadableLocalMoveArg1(DWORD arg1) {
    ...
    if (d000 == StaticToRuntime(0x02178D6C) &&
        d008 == StaticToRuntime(0x02178D60) &&
        d00C == StaticToRuntime(0x02178D50)) {
        return true;
    }
...
bool LooksLikePlayerLocalMoveSelf(void* self) {
    ...
    return d000 == StaticToRuntime(0x02177804) &&
        d314 == 0 &&
        (d1A8 & 0xFF000000) != 0 &&
        coordinatesValid;
}
...
bool LooksLikeUsableStoredPlayerSelf(void* self) {
    DWORD d000 = 0;
    if (!ReadDwordSafe(self, 0x000, &d000) || d000 != StaticToRuntime(0x02177804)) {
        return false;
    }
...
void LogLocalMoveProbeCall(
    ...
    if (internalCall) {
        if (kVerboseHookDiagnostics) {
            ...
            if (callIndex <= 64) {
                ...
                wsprintfA(
                    line,
                    "Local move GOG 0x553000 internal call ignored for capture: self=0x%08X arg1=0x%08X arg2=0x%08X arg3=0x%08X.",
                    reinterpret_cast<DWORD>(self),
                    arg1,
                    arg2,
                    arg3);
    ...
    if (!LooksLikeReadableLocalMoveArg1(arg1)) {
        if (kVerboseHookDiagnostics) {
            ...
            wsprintfA(
                line,
                "Local move GOG 0x553000 capture skipped: unreadable arg1 self=0x%08X arg1=0x%08X.",
                reinterpret_cast<DWORD>(self),
                arg1);
            LogLine(line);
        }
    ...
    ReadDirectPlayerContextFromController(
        ...
        &directLink,
        &directArg1);
    const bool confirmedPlayerCallsite = sourceCallerStatic == 0x00426E8E;
    ...
    wsprintfA(
        line,
        "Local move GOG 0x553000 captured: self=0x%08X arg1=0x%08X arg2=0x%08X arg3=0x%08X arg4=0x%08X arg5=0x%08X.",
        reinterpret_cast<DWORD>(self),
        arg1,
        arg2,
        arg3,
...
void CallLocalMove(void* self, DWORD arg1, DWORD arg2, DWORD arg3, DWORD arg4, DWORD arg5) {
    ...
    LocalMoveFn localMove = reinterpret_cast<LocalMoveFn>(reinterpret_cast<BYTE*>(exe) + 0x153000);
...
SendLocalMove:
    ...
    // The confirmed vanilla player callsite returning at 0x00426E8E pushes
...
bool ResolveInteractionTargetClearApi() {
    ...
    if (memcmp(primary, expectedPrimary, sizeof(expectedPrimary)) != 0 ||
        ...
        InterlockedExchange(&g_interactionTargetApiState, -1);
        LogLine("Interaction-facing reset disabled: target-clear function bytes do not match the GOG executable.");
        return false;
    }
...
void ExecutePendingMovementOnGameWindowThread() {
    ...
    // The confirmed vanilla player callsite returning at 0x00426E8E also pushes
...
void InstallStationaryClickFallback() {
    ...
    BYTE* target = reinterpret_cast<BYTE*>(exe) + 0x2ABA9;
    ...
    for (DWORD i = 0; i < sizeof(expected); ++i) {
        if (target[i] != expected[i]) {
            ...
            LogLine("Stationary click fallback install skipped: target bytes do not match GOG executable.");
            return;
        }
    }
    ...
    g_stationaryFallbackAttackTarget = reinterpret_cast<BYTE*>(exe) + 0x2AB8B;
    g_stationaryFallbackContinueTarget = reinterpret_cast<BYTE*>(exe) + 0x2ABB1;
    ...
    *reinterpret_cast<DWORD*>(target + 1) =
    ...
    LogLine("Stationary click fallback installed at Torchlight2.exe+0x2ABA9.");
}
...
void InstallMessageSendProbe() {
    ...
    BYTE* target = reinterpret_cast<BYTE*>(exe) + 0x2E18B0;
    ...
    for (DWORD i = 0; i < sizeof(expectedPrefix); ++i) {
        if (target[i] != expectedPrefix[i]) {
            ...
            LogLine("Transition message hook install skipped: target prefix does not match GOG executable.");
            return;
        }
    }
    for (DWORD i = 0; i < sizeof(expectedSuffix); ++i) {
        if (target[i + 7] != expectedSuffix[i]) {
            LogBytesAt("Transition message hook target bytes:", target, 16);
            LogLine("Transition message hook install skipped: target bytes do not match GOG executable.");
            return;
        }
    }

    if (InstallRelativeJumpHook(target, MessageSendProbeHook, g_messageProbeOriginal, 7, &g_messageProbeGateway)) {
        LogLine("Transition message hook installed at Torchlight2.exe+0x2E18B0.");
    } else {
