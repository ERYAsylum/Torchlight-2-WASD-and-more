# TL2_WASD_and_more v1.0.02z1-gog - GOG

## English

Unofficial DLL mod for the GOG build of Torchlight II. It adds camera-relative keyboard movement, disables click-to-move without breaking combat/interactions, and includes an optional active range x1.5 engine patch.
...
This branch targets the GOG executable:

- Game: Torchlight II
- Build: GOG
- Executable version observed during development: `1.25.9.5`
- Expected install directory example: `C:\Games\Torchlight 2 gog`

Do not use this GOG build on the Steam executable. The Steam and GOG executables have different code layouts and require different hook addresses.
...
2. Download the GOG binary release zip.
3. Extract or copy the three mod files from that zip into the Torchlight II GOG directory next to `Torchlight2.exe`:
...
The GOG executable is not a drop-in replacement for the Steam executable. This port uses GOG-specific addresses for:

- `LocalMove`.
- transition message hooks.
- stationary click fallback.
- player and movement-context fingerprints.
- CEGUI overlay lifecycle hooks.
- interaction-facing target clear functions.
- active range x1.5 patch sites.

The relevant byte fingerprints were checked against `C:\Games\Torchlight 2 gog\Torchlight2.exe` before the Release Win32 build. See `PORT_NOTES_GOG_v1.0.02z1.txt` in the parent folder for the detailed offset list.
...
Mod DLL non officiel pour la version GOG de Torchlight II. Il ajoute un déplacement clavier relatif à la caméra, désactive le click-to-move sans casser les combats/interactions, et inclut un patch optionnel de portée active x1.5.
...
Cette branche cible l'exécutable GOG :

- Jeu : Torchlight II
- Version : GOG
- Version d'exécutable observée pendant le développement : `1.25.9.5`
- Exemple de dossier d'installation attendu : `C:\Games\Torchlight 2 gog`

N'utilisez pas cette version GOG sur l'exécutable Steam. Les exécutables Steam et GOG ont une disposition mémoire différente et demandent des adresses de hooks différentes.
...
2. Téléchargez le zip binaire de la release GOG.
3. Extrayez ou copiez les trois fichiers du mod depuis ce zip dans le dossier GOG de Torchlight II, à côté de `Torchlight2.exe` :
   - `TL2TrueKeyboardMove.dll`
   - `TL2TrueKeyboardMove.ini`
   - `MSWSOCK.dll`
...
### Notes du portage GOG

L'exécutable GOG n'est pas interchangeable avec l'exécutable Steam. Ce port utilise des adresses spécifiques à GOG pour :

- `LocalMove`.
- les hooks de messages de transition.
- le fallback de clic stationnaire.
- les empreintes du joueur et du contexte de mouvement.
- les hooks de cycle de vie overlay CEGUI.
- les fonctions de nettoyage de cible d'interaction.
- les sites du patch de portée active x1.5.

Les empreintes d'octets concernées ont été vérifiées contre `C:\Games\Torchlight 2 gog\Torchlight2.exe` avant la compilation Release Win32. Voir `PORT_NOTES_GOG_v1.0.02z1.txt` dans le dossier parent pour la liste détaillée des offsets.
